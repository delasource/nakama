...
func main() {
	...
	sessionCache := server.NewLocalSessionCache(config.GetSession().TokenExpirySec, config.GetSession().EnableTokenWhitelist)
	consoleSessionCache := server.NewLocalSessionCache(config.GetConsole().TokenExpirySec, false)
